...
Small bug (line 404), you used `cap` instead of `league.object[["cap"]]`, which relied on the variable to exist outside of the function.

In my particular data set, two kickers are tied at 10th place.  No one checked if this could occur, so (in the default example) everyone ends up with a 81x6 data.frame.  You restrict the number of rows to `nteams*player.each.team`.  In my example this removes the 10th `te` and keeps 11 `k`.  I won't bother counting off as this was an extremely rare event but it did negatively affect your `league` object more than others.

#### Task 2, -3 points

Fails to handle unusual input.

#### Task 3.1, -2 points

Unfortunately `sims` is ordered by position and `sim` is ordered by name, so your merge fails.  This would work:

`sims[,i+3] <- sim[match(sims[,'PlayerName'], sim[,'PlayerName']), 'value']`

I would have preferred you return a `league` object:

```{r eval=FALSE}
league.object[["sims"]] <- sims
return(league.object)
```

#### Task 3.4, -1 point

Plot should handle `league.conf.interval` object with any probs (ie c(0.1,0.2,0.5,0.8)).
